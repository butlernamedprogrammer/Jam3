...
namespace Opsive.UltimateInventorySystem.UI.Item
{
    ...
    public class ItemViewSlotCursorManager : MonoBehaviour, IObjectWithIDReadOnly
    {
        ...
        public ItemViewSlot SourceItemViewSlot
        {
            get
            {
                //Just to be safe, refresh the source view slot in case it moved index.
                RefreshSourceViewSlot();
                return m_SourceItemViewSlot;
            }
        }
        ...
        public void StartMove(ItemViewSlotEventData sourceSlotEventData, Vector2 position, bool usingDrag)
        {
            if (CanMove() == false) { return; }

            m_UsingDrag = usingDrag;
        ...
        public void StartMove(ItemViewSlotEventData slotEventData)
        {
            ...
            m_SourceSlotEventData = slotEventData;
        ...
        protected void StartMove(ItemViewSlot itemViewSlot, ItemViewSlotsContainerBase viewSlotsContainer)
        {
            ...
            if (m_SourceContainer != null) {
                ...
                //In some cases the source view slot might move index which
                //Example: when the InventoryGrid scroll offset changes.
                m_SourceContainer.OnDraw += RefreshSourceViewSlot;
            }
        ...
        /// <summary>
        /// In some cases the source view slot might move index which means it needs to be updated
        /// Example: when the InventoryGrid scroll offset changes.
        /// </summary>
        public void RefreshSourceViewSlot()
        {
            if(m_SourceSlotEventData == null){ return; }
            
            var currentIndex = m_SourceSlotEventData.SlotIndex + m_SourceContainer?.SlotIndexOffset ?? 0;
            var previousIndex = m_SourceSlotEventData.ItemIndex;
            ...
            var newSlotIndex = m_SourceSlotEventData.SlotIndex + previousIndex - currentIndex;

            //Compare the previous and new view slot.
            var previousSourceViewSlot = m_SourceItemViewSlot;
            var newSourceViewSlot = m_SourceContainer.GetItemViewSlot(newSlotIndex);

            if (previousSourceViewSlot == newSourceViewSlot) {
                return;
            }
            
            //Change the moving source
            if (previousSourceViewSlot != null) {
                SetItemViewAsMovingSource(previousSourceViewSlot, false);
            }

            m_SourceItemViewSlot = newSourceViewSlot;
