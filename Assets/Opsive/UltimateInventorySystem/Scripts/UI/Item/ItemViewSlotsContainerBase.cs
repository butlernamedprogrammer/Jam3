...
namespace Opsive.UltimateInventorySystem.UI.Item
{
    ...
    public abstract class ItemViewSlotsContainerBase : MonoBehaviour
    {
        ...
        //In some cases the items shown might be a subset of all the items (example InventoryGrid.)
        //This offset returns which is the first item to be shown in the slot container
        public virtual int SlotIndexOffset => 0;
        ...
        {
            if (index < 0 || index >= m_ItemViewSlots.Length) { return null; }
        ...
        /// </summary>
        /// <param name="itemInfo">The item info to check for.</param>
        /// <returns>True if the item is contained.</returns>
        public bool ContainsItem(ItemInfo itemInfo)
        {
            return GetItemIndex(itemInfo) != -1;
        }
